...
-- Assuming images are stored as absolute URLs, we try to make them server-agnostic or point to storage.

-- Check and update products
UPDATE products
SET image_url = REPLACE(image_url, 'http://localhost:5173', 'https://vedalife-ayurveda.vercel.app')
WHERE image_url LIKE 'http://localhost:5173%';

-- Check and update product_variants (if they have images)
-- (Assuming they don't have an image_url column based on schema seen, but good to check if added)
