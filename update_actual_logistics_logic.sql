-- Update update_order_status to capture ACTUAL timestamps
...
BEGIN
  ...
  -- If marking Shipped, record actual pickup time
  IF p_new_status = 'Shipped' THEN
      UPDATE orders 
      SET actual_pickup = NOW() 
      WHERE id = p_order_id;
      -- Note: We do NOT touch scheduled_pickup here. That is the plan.
  END IF;

  -- If marking Delivered, record actual delivery time
  IF p_new_status = 'Delivered' THEN
      UPDATE orders 
...
-- Update verify_delivery (App usage)
...
BEGIN
  ...
  IF v_correct_otp = p_otp OR v_correct_otp IS NULL THEN 
     ...
     -- Update Order Status AND Actual Delivery Time
     UPDATE orders 
     SET status = 'Delivered',
         payment_status = 'Paid',
         actual_delivery = NOW() -- Always set actual time on verification
     WHERE id = p_order_id;
